function create_dungeon_room(dungeon_array) {
    ...
    layer_set_target_room(new_room);
    {
        ...
        var tilemap_floor_id = layer_tilemap_create(layer_floor_id, 0, 0, global.tileset_floor_index, tile_size, tile_size);
        tilemap_clear(tilemap_floor_id, -1);

        // Wall layer
        var layer_walls_id = layer_create(2, "Walls");
        var tilemap_walls_id = layer_tilemap_create(layer_walls_id, 0, 0, global.tileset_walls_index, tile_size, tile_size);
        ...
        for (var dy = 0; dy < dungeon_height; dy++) {
            for (var dx = 0; dx < dungeon_width; dx++) {
                if (dungeon_array[dy][dx] == 1) {
                    tilemap_set(tilemap_walls_id, dx, dy, global.wall_tile_index);
                } else {
                    tilemap_set(tilemap_floor_id, dx, dy, global.floor_tile_index);
                }
            }
        }
    }
